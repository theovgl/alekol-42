		...
		if ((interaction.inGuild() &&
			...
			row.addComponents(
				new MessageButton()
					.setLabel('Register')
					.setStyle('LINK')
					.setURL(`https://api.intra.42.fr/oauth/authorize?client_id=${process.env.UID_42}&redirect_uri=${encodeURIComponent(process.env.REDIRECT_URI + '/register')}&response_type=code&state=${state}`),
			);
		} else {
